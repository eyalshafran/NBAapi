...
def court(ax=None, color='black', lw=2, outer_lines=False):
    ...
    if ax is None:
        ax = plt.gca(xlim = [-30,30],ylim = [40,-10],xticks=[],yticks=[],aspect=1.0)

    # Create the various parts of an NBA basketball court

    # Create the basketball hoop
    # Diameter of a hoop is 1.5 
    hoop = Circle((0, 0), radius=1.5, linewidth=lw/2, color=color, fill=False)

    # Create backboard
    backboard = Rectangle((-3, -1.5), 6, -0.1, linewidth=lw, color=color)
    ...
    if outer_lines:
        ...
        outer_lines = Rectangle((-25, -5.25), 50, 52.25, linewidth=lw,
    ...
    plt.text(0,38,'By: Doingthedishes',color='black',horizontalalignment='center',fontsize=20)
#    ax.axis('off')
    return ax
...
def zones(**kwargs):
    ...
    ax.plot([np.cos(ang/180*np.pi)*16, np.cos(ang/180*np.pi)*23.75],
             [np.sin(ang/180*np.pi)*16, np.sin(ang/180*np.pi)*23.75],**kwargs)
    ang = 72.0
    ax.plot([np.cos(ang/180*np.pi)*24, np.cos(ang/180*np.pi)*47],
             [np.sin(ang/180*np.pi)*24, 47],**kwargs)
    ang = 72.0+36.0
    ax.plot([np.cos(ang/180*np.pi)*24, np.cos(ang/180*np.pi)*47],
             [np.sin(ang/180*np.pi)*24, 47],**kwargs)
...
def text_in_zone(string,zone,box_alpha = 0.75,**kwargs):
    ...
    if zone == ('Less Than 8 ft.','Center(C)') :
        t = plt.text(0,4,string,horizontalalignment='center',verticalalignment='center',**kwargs)
        ...
        if zone[1] == 'Center(C)':
            ...
            t = plt.text(12,2,string,horizontalalignment='center',verticalalignment='center',**kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        ...
        if zone[1] == 'Center(C)':
            ...
            t = plt.text(19,6.3,string,horizontalalignment='center',verticalalignment='center',**kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] ==  'Left Side(L)':
            t = plt.text(-19,6.3,string,horizontalalignment='center',verticalalignment='center',**kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Right Side Center(RC)':
            t = plt.text(12.5,16.5,string,horizontalalignment='center',verticalalignment='center',**kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        ...
        if zone[1] == 'Center(C)':
            ...
            t = plt.text(17,24,string,horizontalalignment='center',verticalalignment='center',**kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Left Side Center(LC)':
            t = plt.text(-17,24,string,horizontalalignment='center',verticalalignment='center',**kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha)) 
